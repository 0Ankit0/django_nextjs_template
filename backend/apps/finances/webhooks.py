...
def activate_free_plan_on_subscription_deletion(event: djstripe_models.Event, **kwargs):
    """
...
def capture_release_schedule(event: djstripe_models.Event, **kwargs):
    """
...
def update_subscription_default_payment_method(event: djstripe_models.Event, **kwargs):
    """
...
def remove_detached_payment_method(event: djstripe_models.Event, **kwargs):
    obj = event.data["object"]
    djstripe_models.PaymentMethod.objects.filter(id=obj["id"]).delete()


@djstripe_receiver(["invoice.payment_failed", "invoice.payment_action_required"])
def cancel_trial_subscription_on_payment_failure(event: djstripe_models.Event, **kwargs):
    obj = event.data["object"]
...
@djstripe_receiver(["invoice.payment_failed", "invoice.payment_action_required"])
def send_email_on_subscription_payment_failure(event: djstripe_models.Event, **kwargs):
    """
...
def charge_refund_updated(event: djstripe_models.Event, **kwargs):
    """
