...
class TestUserPostSaveSignal:
    ...
    def test_signal_is_not_raising_exception_on_auth_error(self, tenant_factory, mocker):
        mock = mocker.patch("finances.services.subscriptions.initialize_tenant", side_effect=AuthenticationError())
        sentry_mock = mocker.patch("finances.signals.logger.error")
    ...
    def test_reraise_stripe_error(self, tenant_factory, totp_mock, mocker):
        ...
        mocker.patch("finances.services.subscriptions.initialize_tenant", side_effect=initial_error())
